...
{
	/* Die Material-Eigenschaften anwenden
	 * (diffuse, specular, shininess und ambient).
	 */
...
void drawGround()
{
	...
	glBegin(GL_QUADS);
		...
		for(int x = -resolution; x < resolution; ++x)
		{
			...
			{	
				glVertex3f((x + 1) / (float)resolution * size, 0,
						(y + 1) / (float)resolution * size);
				glVertex3f((x + 1) / (float)resolution * size, 0,
						y / (float)resolution * size);
				glVertex3f(x / (float)resolution * size, 0,
						y / (float)resolution * size);
				glVertex3f(x / (float)resolution * size, 0,
						(y + 1) / (float)resolution * size);
			}
...
void display(void)	
{
	...
	/* Platzieren eines Punktlichtes, das sich auf einer Kreisbahn mit
	 * Radius 2.5f in der Höhe 0.5 um (0,0,0) dreht.
	 * Die Intensität soll quadratisch mit dem Faktor 0.6 abnehmen.
	 * (Hinweis: Nutzen Sie dazu die Quadratic Attentuation)
	 */
	...
	/* Plazieren eines Richtungslichtes aus der Richtung (1,1,1) mit einer
	 * Diffusfarbe von (0.2f, 0.2f, 0.2f)
	 */
	 ...
	 * Cutoff-Winkel von 45°, einem Spot-Exponent von 16 und mit der
	 * Diffusfarbe 'Orange'.
	 */
	...
	/* Grünen Torus rendern bei Position(-3.0f, 0.5f, 3.0f) mit innerem Radius
	 * 0.15f, äußerem Radius 0.35f und jeweils 32 Seiten und Ringen
	 */
	applyMaterial(Green);
	glPushMatrix();
		glTranslatef(-3.0f, 0.5f, 3.0f);
	...
	/* Violette (Purple) Kugel rendern bei Position(-3.0f, 0.5f, -3.0f) mit
	 * Radius 0.5f und jeweils 32 Längen- und Breitensegmenten
	 */
	...
	/* Blauen AUFRECHTEN Kegel rendern bei Position(3.0f, 0.0f, -3.0f) mit
	 * Basisradius 0.5, Höhe 1, 32 Ringsegmenten und 4 Höhensegmenten
	 */
	applyMaterial(Blue);
	glPushMatrix();
